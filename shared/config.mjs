...
export const ABI = parseAbi([
  ...
  'struct Player { uint48 total; uint48 spent; uint32 lastBlock; uint32 boostUntil; uint16 round; uint16 rate; uint16 power; uint8 mega; uint8 farm; uint8 combo; uint8 magnet; }',
  ...
  'function megaCost(uint24 level) pure returns (uint64)',
  'function farmCost(uint24 level) pure returns (uint64)',
  'function comboCost(uint24 level) pure returns (uint64)',
  ...
  'event PlayerUpdated(address indexed player, uint32 round, uint64 total, uint64 spent, uint24 rate, uint24 power, uint32 extra, uint40 boostUntil)',
  'error NotHost()',
  'error NotJoined()',
  'error RoundNotLive()',
  'error BadCount()',
...
export const KIND_MEGA = 2
export const KIND_FARM = 3
export const KIND_COMBO = 4
export const KIND_MAGNET = 5
...
export const UPGRADES = [
  ...
  { kind: KIND_MEGA, key: 'mega', icon: '🍋', label: 'Espresso Sigma', base: 0, max: 50, cost: quad(150n), detail: (n) => `+5 aura par tap · niv. ${n}` },
  { kind: KIND_FARM, key: 'farm', icon: '🏭', label: 'Ferme à Aura', base: 0, max: 50, cost: quad(250n), detail: (n) => `+8 aura par bloc · niv. ${n}` },
  { kind: KIND_COMBO, key: 'combo', icon: '🔥', label: 'Combo Mama Mia', base: 0, max: 20, cost: quad(400n), detail: (n) => `+10 % sur tes taps · niv. ${n}` },
...
export const levelsOf = (p, round) => ({
  power: 1, rate: 0, mega: 0, farm: 0, combo: 0, magnet: 0, boostUntil: 0,
  ...(p && p.round === round ? p : {}),
})
/** Aura passive par bloc (miroir de _settle). */
export const passivePerBlock = (p) => p.rate + 8 * p.farm
/** Aura d'un tap (miroir de tap() ; fractionnaire pour l'affichage). */
export const tapValue = (p, boosted) => ((p.power + 5 * p.mega) * (100 + 10 * p.combo) * (boosted ? BOOST_MULT : 1)) / 100
